...
export default function ButtonBar({ name, to, index, onChangePath }: ButtonBarProps) {
  ...
  return (
    <Stack direction="row">
      ...
      <Box component='span' sx={{ padding: '3px', fontWeight: 600, textAlign: 'center', cursor: 'pointer', width: '100%', textOverflow: 'ellipsis', overflow: 'hidden', whiteSpace: 'nowrap' }} onClick={sendToPath}>
        {name}
      </Box>
    </Stack>
  );
