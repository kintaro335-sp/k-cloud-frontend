...
import { Box, Chip, Button, Stack, IconButton, Menu, MenuItem, Grid } from '@mui/material';
...
export default function SelectScopes({ onChange, scopes }: SelectScopesProps) {
  ...
  return (
    <Box>
      <Button ref={anchorRef} onClick={clickOpen}>
        <Trans i18nKey="pages.apikeys.form.label_add_scope">Agregar Scopes</Trans>
      </Button>
      <Menu
        anchorEl={anchorRef.current}
        open={open}
        onClose={clickClose}
        MenuListProps={{ sx: { maxHeight: '300px' } }}
      >
        {scopeList
      ...
      <Stack spacing={1} direction="row" sx={{ maxWidth: '700px', overflow: 'auto', padding: '14px' }}>
        {scopes.map((scope) => (
          <Chip
            key={scope}
            label={
